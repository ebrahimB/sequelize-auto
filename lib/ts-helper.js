...
function doCase(word, caseType){
  ...
  caseType = caseType !== null & caseType != undefined ? caseType.toLowerCase() : null;
  switch (caseType) {
    case "pascal":  
...
function generateTableModels(tableNames, isSpaces, indentation, caseType, caseForFileType) {
  var spaces = '';
  ...
  function generateInterface() {
    ...
    for (var i = 0; i < tableNames.length; i++) {
      var table = doCase(tableNames[i], caseType);
  ...
  function generateTableMapper() {
    ...
    for (var i = 0; i < tableNames.length; i++) {
      var tableForClass = doCase(tableNames[i], caseType);

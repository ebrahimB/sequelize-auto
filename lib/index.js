...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      var tableName = tsHelper.func.doCase(table, self.options.case);
      ...
      _.each(fields, function(field, i){
        ...
        var fieldName = tsHelper.func.doCase(field, self.options.case);
      ...
      if(self.options.typescript) typescriptFiles[1] = tsHelper.model.generateTableModels(_.keys(text), self.options.spaces, self.options.indentation, self.options.case, self.options.caseForFileName);

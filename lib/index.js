...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      var tableName = tsHelper.func.doCase(table, self.options.caseModel);
      var tsTableDef = self.options.typescript || self.options.typescriptOnly ? 'export interface ' + tableName + 'Attribute {' : '';

      if(!self.options.typescript && !self.options.typescriptOnly){
      ...
      _.each(fields, function(field, i){
        ...
        var fieldName = tsHelper.func.doCase(field, self.options.caseField);
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if(attr === "autoIncrement"){
            var autoIncrement = self.tables[table][field][attr];
            if(!autoIncrement){              
              return true
            }
            text[table] += spaces + spaces + spaces + "autoIncrement: " + autoIncrement;
          }else if(attr === "comment"){
            var comment = self.tables[table][field][attr]; 
            if(!comment){
              return true;
            }
            comment = comment.replace(/(\r\n|\n|\r)/gm," ");
            text[table] += spaces + spaces + spaces + "comment: " + "\"" + comment + "\"";
            ...
            if(self.options.typescript || self.options.typescriptOnly) tsAllowNull = self.tables[table][field][attr];
          }
            ...
            var rawAttr = self.tables[table][field][attr];
            var _attr = (rawAttr || '').toString().toLowerCase();
            var val = quoteWrapper + self.tables[table][field][attr] + quoteWrapper;

            if (_attr === "boolean" || _attr === "bit(1)" || _attr === "bit") {
              val = 'dataTypes.BOOLEAN';
            }
            else if (_attr.match(/^(bigint|smallint|mediumint|tinyint|int)/)) {
              var length = _attr.match(/\(\d+\)/);
              if(_attr.match(/^int/)){
                val = 'dataTypes.INTEGER';
              }else if(_attr.match(/^bigint/)){
                val = 'dataTypes.BIGINT';
              }else if(_attr.match(/^mediumint/)){
                val = 'dataTypes.MEDIUMINT';
              }else if(_attr.match(/^smallint/)){
                val = 'dataTypes.SMALLINT';
              }else if(_attr.match(/^tinyint/)){
                val = 'dataTypes.TINYINT';
              }
            ...
            if(self.options.typescript || self.options.typescriptOnly) tsVal = val;
          }
        ...
        if(self.options.typescript || self.options.typescriptOnly) tsTableDef += tsHelper.def.getMemberDefinition(spaces, fieldName, tsVal, tsAllowNull);
      });
      ...
      if(self.options.typescriptOnly) typescriptFiles[0] += tsHelper.def.getTableDefinition(tsTableDef, tableName, true);
      ...
      if(self.options.typescript) typescriptFiles[1] = tsHelper.model.generateTableModels(_.keys(text), self.options.spaces, self.options.indentation, self.options.caseModel, self.options.caseForFileName);
...
AutoSequelize.prototype.write = function(attributes, typescriptFiles, callback) {
  ...
  if (self.options.typescriptOnly) {
  ...
  function createFile(table, _callback) {
    var fileName = tsHelper.func.doCase(table, self.options.caseForFileName);
    if(!self.options.typescriptOnly){
